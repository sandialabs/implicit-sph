...
class Atom : protected Pointers {
 public:
  ...
  // for ale and high order time stepping (bdf)
  double **xdot;
  double ***vprev, ***xprev;

  // for fluid material
  // - the name says what it is
  double *viscosity, *pressure, *density;
  int  pressure_flag, viscosity_flag; // density_flag is rmass_flag

  // for solute transport
  double **concentration;
  int concentration_flag;

  // for electrostatics:
  // - eps - dielectric property 
  // - sigma - conductivity property 
  double *eps, *sigma;
  int eps_flag, sigma_flag;

  // for poisson boltzmann:
  // - psi - potential
  // - psi0 - prescribed potential on boundary
  // - psigrad - gradient of psi which used to compute electric field
  double *psi, *psi0, **psigrad;
  int psi_flag, psi0_flag, psigrad_flag;

  // for background field:
  // - phi - potential
  // - phigrad - gradient of phi
  double *phi, **phigrad;
  int phi_flag, phigrad_flag;
