...
enum{TYPE,TYPE_FRACTION,MOLECULE,X,Y,Z,CHARGE,MASS,SHAPE,LENGTH,TRI,
     ...
     ISPH_PRESSURE,ISPH_DENSITY,ISPH_VISCOSITY,ISPH_EPS,ISPH_PSI0,ISPH_SIGMA,
     ISPH_CONCENTRATION_BEGIN,
     ISPH_CONCENTRATION_END=ISPH_CONCENTRATION_BEGIN+ISPH_MAX_CONCENTRATION};
#else
enum{TYPE,TYPE_FRACTION,MOLECULE,X,Y,Z,CHARGE,MASS,SHAPE,LENGTH,TRI,
     DIPOLE,DIPOLE_RANDOM,QUAT,QUAT_RANDOM,THETA,ANGMOM,
     DIAMETER,DENSITY,VOLUME,IMAGE,BOND,ANGLE,DIHEDRAL,IMPROPER,
...
void Set::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    if (strcmp(arg[iarg],"type") == 0) {
      ...
      iarg += 2;
#ifdef HAVE_IMPLICIT_SPH
    } else if (strcmp(arg[iarg],"isph_pressure") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      ...
      set(ISPH_PRESSURE);
      iarg += 2;
    } else if (strcmp(arg[iarg],"isph_viscosity") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      ...
      set(ISPH_VISCOSITY);
      iarg += 2;
    } else if (strcmp(arg[iarg],"isph_density") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      if (!atom->rmass_flag)
      ...
      set(ISPH_DENSITY);
      iarg += 2;
    } else if (strcmp(arg[iarg],"isph_dielectric") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      if (!atom->eps_flag)
        error->all(FLERR,"Cannot set this attribute for this atom style");
      if (dvalue <= 0.0) error->all(FLERR,"Invalid dielectric in set command");
      set(ISPH_EPS);
      iarg += 2;
    } else if (strcmp(arg[iarg],"isph_conductivity") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      if (!atom->sigma_flag)
      ...
      set(ISPH_SIGMA);
      iarg += 2;
    } else if (strcmp(arg[iarg],"isph_electric_potential_on_wall") == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      if (!atom->psi0_flag)
        error->all(FLERR,"Cannot set this attribute for this atom style");
      // if (dvalue <= 0.0) error->all(FLERR,"Invalid electric potential in set command");
      set(ISPH_PSI0);
      iarg += 2;
    } else if (strncmp(arg[iarg],"isph_concentration", 18) == 0) {
      if (iarg+2 > narg) error->all(FLERR,"Illegal set command");
      if (strstr(arg[iarg+1],"v_") == arg[iarg+1]) varparse(arg[iarg+1],1);
      else dvalue = force->numeric(FLERR,arg[iarg+1]);
      if (!atom->concentration_flag)
        error->all(FLERR,"Cannot set this attribute for this atom style");
      // if (dvalue <= 0.0) error->all(FLERR,"Invalid electric potential in set command");

      const string cid_string(arg[iarg]);
      const string cid_prefix("isph_concentration_");

      bool is_cid_string_found = false;
      for (int cid=0;cid<ISPH_MAX_CONCENTRATION;++cid) {
        if (cid_string.find(cid_prefix+to_string(cid)) != string::npos) {
          is_cid_string_found = true;
          set(ISPH_CONCENTRATION_BEGIN+cid);
          break;
        }
      }
      if (!is_cid_string_found)
        set(ISPH_CONCENTRATION_BEGIN);
      iarg += 2;
#endif
...
#ifdef HAVE_IMPLICIT_SPH
    else if (keyword == ISPH_PRESSURE) atom->pressure[i] = dvalue;
    else if (keyword == ISPH_DENSITY) {
      if (dvalue < 0.0) error->one(FLERR,"Invalid diameter in set command");
      atom->density[i] = dvalue;
    }
    else if (keyword == ISPH_VISCOSITY) {
      if (dvalue < 0.0) error->one(FLERR,"Invalid diameter in set command");
      atom->viscosity[i] = dvalue;
    }

    else if (keyword == ISPH_EPS) {
      if (dvalue < 0.0) error->one(FLERR,"Invalid diameter in set command");
      atom->eps[i] = dvalue;
    }
    else if (keyword == ISPH_SIGMA) {
      if (dvalue < 0.0) error->one(FLERR,"Invalid diameter in set command");
      atom->sigma[i] = dvalue;
    }
    else if (keyword == ISPH_PSI0) {
      // if (dvalue < 0.0) error->one(FLERR,"Invalid diameter in set command");
      atom->psi0[i] = dvalue;
    }
    else if (keyword >= ISPH_CONCENTRATION_BEGIN &&
             keyword <  ISPH_CONCENTRATION_END) {
      // if (dvalue < 0.0) error->one(FLERR,"Invalid diameter in set command");
      atom->concentration[keyword-ISPH_CONCENTRATION_BEGIN][i] = dvalue;
    }
#endif
