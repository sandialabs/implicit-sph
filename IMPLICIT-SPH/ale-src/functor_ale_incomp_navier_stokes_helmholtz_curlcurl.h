#pragma once
#ifndef __FUNCTOR_ALE_INCOMP_NAVIER_STOKES_HELMHOLTZ_CURLCURL_H__
#define __FUNCTOR_ALE_INCOMP_NAVIER_STOKES_HELMHOLTZ_CURLCURL_H__
...
namespace LAMMPS_NS {
  ...
  class FunctorOuterAleIncompNavierStokesHelmholtzCurlCurl : public FunctorOuter<PairIsph> {
  public:
    FunctorOuterAleIncompNavierStokesHelmholtzCurlCurl(PairIsph *isph, 
  ...
  template<class PairIsph,
           ...
           template<typename> class CurlCurl>
  inline void
  FunctorOuterAleIncompNavierStokesHelmholtzCurlCurl<PairIsph,
                                                     LaplacianMatrix,
                                                     GradientOperator,
                                                     CurlCurl>::enterFor() {
    // --- Error check --------------------------------------------------------------------- 
    if (_filter != NULL)
      _pair->error->all(FLERR, "FunctorAleIncompNavierStokesHelmholtzCurlCurl:: Filter is not allowed");

    if (_pair->A.is_filled == 1)
      _pair->error->all(FLERR, "FunctorIncompNavierStokesHelmholtzCurlCurl:: A is already filled");
    // --- Error check --------------------------------------------------------------------- 

    // compute laplacin/advection matrix
    switch (_pair->boundary_particle) {
    case PairIsph::Solid: {
      _pair->error->all(FLERR, "FunctorAleIncompNavierStokesHelmholtzCurlCurl:: SPH CurlCurl is not implemented");
      break;
    }
  ...
  FunctorOuterAleIncompNavierStokesHelmholtzCurlCurl<PairIsph,
                                                     LaplacianMatrix,
                                                     GradientOperator,
                                                     CurlCurl>::operator()(const int ii) {
    int i = _ilist[ii];
    int itype = _type[i];
    ...
    default:
      _pair->error->all(FLERR, "FunctorAleIncompNavierStokesHelmholtzCurlCurl:: Particle types are not supported");     
    }
  }
