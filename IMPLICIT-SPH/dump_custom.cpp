...
enum{ID,MOL,PROC,PROCP1,TYPE,ELEMENT,MASS,
     ...
     TQX,TQY,TQZ,
     VFRAC,PRESSURE,PSI,PSI0,PSIGRADX,PSIGRADY,PSIGRADZ,PHI,PHIGRADX,PHIGRADY,PHIGRADZ,
     CONDUCTIVITY,DIELECTRICITY,DENSITY,VISCOSITY, // not yet hooked up
...
#ifdef HAVE_IMPLICIT_SPH
      } else if (thresh_array[ithresh] == VFRAC) {
        if (!atom->vfrac_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = atom->vfrac;
        nstride = 1;
      } else if (thresh_array[ithresh] == PRESSURE) {
        if (!atom->pressure_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = atom->pressure;
        nstride = 1;
      } else if (thresh_array[ithresh] == PSI) {
        if (!atom->psi_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = atom->psi;
        nstride = 1;
      } else if (thresh_array[ithresh] == PSI0) {
        if (!atom->psi_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = atom->psi0;
        nstride = 1;
      } else if (thresh_array[ithresh] == PSIGRADX) {
        if (!atom->psigrad_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = &atom->psigrad[0][0];
        nstride = 3;
      } else if (thresh_array[ithresh] == PSIGRADY) {
        if (!atom->psigrad_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = &atom->psigrad[0][1];
        nstride = 3;
      } else if (thresh_array[ithresh] == PSIGRADZ) {
        if (!atom->psigrad_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = &atom->psigrad[0][2];
        nstride = 3;
      } else if (thresh_array[ithresh] == PHI) {
        if (!atom->phi_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = atom->phi;
        nstride = 1;
      } else if (thresh_array[ithresh] == PHIGRADX) {
        if (!atom->phigrad_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = &atom->phigrad[0][0];
        nstride = 3;
      } else if (thresh_array[ithresh] == PHIGRADY) {
        if (!atom->phigrad_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = &atom->phigrad[0][1];
        nstride = 3;
      } else if (thresh_array[ithresh] == PHIGRADZ) {
        if (!atom->phigrad_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = &atom->phigrad[0][2];
        nstride = 3;
...
#ifdef HAVE_IMPLICIT_SPH
    } else if (strcmp(arg[iarg],"vfrac") == 0) {
      pack_choice[i] = &DumpCustom::pack_vfrac;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"pressure") == 0) {
      pack_choice[i] = &DumpCustom::pack_pressure;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"psi") == 0) {
      pack_choice[i] = &DumpCustom::pack_psi;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"psigradx") == 0) {
      pack_choice[i] = &DumpCustom::pack_psigradx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"psigrady") == 0) {
      pack_choice[i] = &DumpCustom::pack_psigrady;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"psigradz") == 0) {
      pack_choice[i] = &DumpCustom::pack_psigradz;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"phi") == 0) {
      pack_choice[i] = &DumpCustom::pack_phi;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"phigradx") == 0) {
      pack_choice[i] = &DumpCustom::pack_phigradx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"phigrady") == 0) {
      pack_choice[i] = &DumpCustom::pack_phigrady;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"phigradz") == 0) {
      pack_choice[i] = &DumpCustom::pack_phigradz;
      vtype[i] = DOUBLE;
#endif
...
#ifdef HAVE_IMPLICIT_SPH
    else if (strcmp(arg[1],"vfrac") == 0) thresh_array[nthresh] = VFRAC;
    else if (strcmp(arg[1],"pressure") == 0) thresh_array[nthresh] = PRESSURE;
    else if (strcmp(arg[1],"psi") == 0) thresh_array[nthresh] = PSI;
    else if (strcmp(arg[1],"psi0") == 0) thresh_array[nthresh] = PSI0;
    else if (strcmp(arg[1],"psigradx") == 0) thresh_array[nthresh] = PSIGRADX;
    else if (strcmp(arg[1],"psigrady") == 0) thresh_array[nthresh] = PSIGRADY;
    else if (strcmp(arg[1],"psigradz") == 0) thresh_array[nthresh] = PSIGRADZ;
    else if (strcmp(arg[1],"phi") == 0) thresh_array[nthresh] = PHI;
    else if (strcmp(arg[1],"phigradx") == 0) thresh_array[nthresh] = PHIGRADX;
    else if (strcmp(arg[1],"phigrady") == 0) thresh_array[nthresh] = PHIGRADY;
    else if (strcmp(arg[1],"phigradz") == 0) thresh_array[nthresh] = PHIGRADZ;
...
void DumpCustom::pack_vfrac(int n)
{
...
void DumpCustom::pack_pressure(int n)
{
  double *pressure = atom->pressure;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = pressure[clist[i]];
    n += size_one;
  }
}
...
void DumpCustom::pack_psi(int n)
{
  double *psi = atom->psi;
...
void DumpCustom::pack_psi0(int n)
{
  double *psi0 = atom->psi0;
...
void DumpCustom::pack_psigradx(int n)
{
  double **psigrad = atom->psigrad;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = psigrad[clist[i]][0];
    n += size_one;
  }
}
...
void DumpCustom::pack_psigrady(int n)
{
  double **psigrad = atom->psigrad;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = psigrad[clist[i]][1];
    n += size_one;
  }
}
...
void DumpCustom::pack_psigradz(int n)
{
  double **psigrad = atom->psigrad;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = psigrad[clist[i]][2];
    n += size_one;
  }
}
...
void DumpCustom::pack_phi(int n)
{
  double *phi = atom->phi;
...
void DumpCustom::pack_phigradx(int n)
{
  double **phigrad = atom->phigrad;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = phigrad[clist[i]][0];
    n += size_one;
  }
}
...
void DumpCustom::pack_phigrady(int n)
{
  double **phigrad = atom->phigrad;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = phigrad[clist[i]][1];
    n += size_one;
  }
}
...
void DumpCustom::pack_phigradz(int n)
{
  double **phigrad = atom->phigrad;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = phigrad[clist[i]][2];
    n += size_one;
  }
}
