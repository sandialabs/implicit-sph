...
fid = fopen('couette-flow-2d.data', 'wt' );
fprintf(fid,'LAMMPS ISPH data file for Couette Flow 2D\n');
fprintf(fid,'%d atoms\n', natoms);
...
cmd=sprintf("sed -i \'/--datafile/c\\variable           dx equal %.8f #--datafile\' couette-flow-2d.lmp", max(dx,dy));
[r,s] = system(cmd);
disp(s);
