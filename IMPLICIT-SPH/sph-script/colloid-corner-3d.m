...
box_half_height = 1.0*ref_length;
...
% solid type
type((X-xlo).^2 + (Y-ylo).^2 + (Z-zlo).^2 < sphere_radius^2) = 2;
type((X-xhi).^2 + (Y-ylo).^2 + (Z-zlo).^2 < sphere_radius^2) = 2;
type((X-xhi).^2 + (Y-yhi).^2 + (Z-zlo).^2 < sphere_radius^2) = 2;
type((X-xlo).^2 + (Y-yhi).^2 + (Z-zlo).^2 < sphere_radius^2) = 2;

type((X-xlo).^2 + (Y-ylo).^2 + (Z-zhi).^2 < sphere_radius^2) = 2;
type((X-xhi).^2 + (Y-ylo).^2 + (Z-zhi).^2 < sphere_radius^2) = 2;
type((X-xhi).^2 + (Y-yhi).^2 + (Z-zhi).^2 < sphere_radius^2) = 2;
type((X-xlo).^2 + (Y-yhi).^2 + (Z-zhi).^2 < sphere_radius^2) = 2;
...
fid = fopen('colloid-corner-3d.data', 'wt' );
...
cmd=sprintf("sed -i \'/--datafile/c\\variable           dx equal %f #--datafile\' colloid-corner-3d.lmp", max([dx,dy,dz]));
[r,s] = system(cmd);
disp(s);
